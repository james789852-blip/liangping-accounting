...
export default function CKMultiClient() {
  const now = new Date()
  const [year, setYear] = useState(now.getFullYear())
  const [monthNum, setMonthNum] = useState(now.getMonth() + 1)
  const [loading, setLoading] = useState(false)
  const [rows, setRows] = useState<MultiCKRow[]>([])
  ...
  const grand = rows.reduce((acc, r) => ({
    memberRevenue: acc.memberRevenue + r.memberRevenue,
    externalRevenue: acc.externalRevenue + r.externalRevenue,
  ...
  }), { memberRevenue: 0, externalRevenue: 0, revenue: 0, food: 0, pack: 0, misc: 0, totalExpense: 0, balance: 0 })

  return (
    <div className="max-w-7xl mx-auto p-4 space-y-4" style={{ background: '#fafafa', minHeight: '100vh' }}>
      <div>
        <h1 className="text-xl font-bold" style={{ color: '#18181b' }}>央廚多店比較</h1>
        <p className="text-xs" style={{ color: '#a1a1aa' }}>一頁看所有央廚該月收入 / 支出 / 淨額</p>
      </div>
            ...
            <table className="text-xs" style={{ minWidth: 'max-content' }}>
              ...
              <tbody>
                ...
                <tr style={{ borderTop: '2px solid #e4e4e7', background: '#fef3c7' }}>
                  ...
                  <td className="px-2 py-2 text-right tabular-nums font-bold">{fmt(grand.memberRevenue)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold">{fmt(grand.externalRevenue)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold" style={{ color: '#16a34a' }}>{fmt(grand.revenue)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold">{fmt(grand.food)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold">{fmt(grand.pack)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold">{fmt(grand.misc)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold" style={{ color: '#be123c' }}>{fmt(grand.totalExpense)}</td>
                  <td className="px-2 py-2 text-right tabular-nums font-bold" style={{ color: grand.balance >= 0 ? '#0369a1' : '#dc2626' }}>{fmt(grand.balance)}</td>
                </tr>
              </tbody>
            </table>
          </div>
