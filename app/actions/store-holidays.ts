...
async function checkHolidayAccess(storeId: string) {
  const ctx = await getAuthContext()
  if (!ctx) return { error: '未登入' as const }
  if (!canAccessStore(ctx, storeId)) return { error: '無權限管理此店家公休' as const }
  return null
}
...
export async function fetchStoreHolidays(storeId: string, from?: string, to?: string) {
  const auth = await checkHolidayAccess(storeId)
  if (auth) return auth
...
export async function addStoreHolidaysRange(storeId: string, dates: string[], note?: string) {
  ...
  if (uniqueDates.length === 0) return { error: '請選擇正確的公休日期' as const }
  const admin = createAdminClient()
  const rows = uniqueDates.map(d => ({ store_id: storeId, holiday_date: d, note: note?.trim() || null }))
...
export async function addBatchStoreHolidays(storeIds: string[], from: string, to: string, note?: string) {
  ...
  const ctx = await getAuthContext()
  if (!ctx) return { error: '未登入' as const }
  if (!ids.every(id => canAccessStore(ctx, id))) return { error: '無權限管理其中一間店家公休' as const }
...
export async function deleteStoreHoliday(id: string) {
  const admin = createAdminClient()
  const { data: holiday } = await admin.from('store_holidays').select('store_id').eq('id', id).maybeSingle()
  if (!holiday?.store_id) return { error: '找不到此公休日' as const }
  const auth = await checkHolidayAccess(holiday.store_id as string)
  if (auth) return auth
  const { error } = await admin.from('store_holidays').delete().eq('id', id).eq('store_id', holiday.store_id)
  if (error) return { error: error.message }
  revalidatePath('/hq/store-overview')
  return { success: true as const }
}
