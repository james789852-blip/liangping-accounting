...
test('Service Worker 僅保留推播，不攔截頁面與靜態資源載入', () => {
  ...
  assert.match(workerSource, /existingClient\.navigate\(destination\)/)
  assert.match(workerSource, /navigatedClient\.focus\(\)/)
  assert.match(workerSource, /self\.clients\.openWindow\(destination\)/)
