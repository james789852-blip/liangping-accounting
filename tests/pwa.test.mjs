...
test('手機頂端只保留端別與通知，HR 和端別切換集中在抽屜最上方', () => {
  ...
  assert.match(notificationSource, /desktopTarget[\s\S]*relative flex h-10 w-10/)
  assert.doesNotMatch(notificationSource, /fixed bottom-6 right-6/)
  assert.doesNotMatch(notificationSource, /top-\[calc\(4\.25rem/)
  assert.match(hqNavSource, /id="hq-desktop-notification-slot"/)
  assert.match(managerNavSource, /id="manager-desktop-notification-slot"/)
  assert.match(hqMobileHeader, /mobile-header-notification[\s\S]*<NotificationCenter desktopTargetId="hq-desktop-notification-slot" \/>/)
  assert.match(managerMobileHeader, /mobile-header-notification[\s\S]*<NotificationCenter desktopTargetId="manager-desktop-notification-slot" \/>/)
