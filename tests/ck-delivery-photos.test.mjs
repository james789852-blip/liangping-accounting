...
test('手機逐步核對會在照片上方提供不遮擋內容的切換與放大', () => {
  ...
  assert.match(reviewSource, /hq-review-dialog-scroll min-h-0 flex-1 overflow-y-auto/)
  assert.match(overviewSource, /pb-\[calc\(1rem\+env\(safe-area-inset-bottom\)\)\]/)
  assert.match(reviewSource, /hq-review-dialog-footer shrink-0 grid grid-cols-2/)
